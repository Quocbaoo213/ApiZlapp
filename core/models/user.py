...
class UserProfile:
    ...
    def from_dict(cls, d: Dict[str, Any]) -> 'UserProfile':
        ...
        return cls(user_id=uid_val, display_name=d.get('displayName') or f'Người dùng {uid_val}', alias=d.get('alias'), username=d.get('uname'), gender=g, dob=dob_val if dob_val else None, phone=d.get('phoneNumber') if d.get('phoneNumber') else None, is_friend=d.get('isFr', 0) == 1 or d.get('is_friend', False), is_blocked=d.get('isBlock', 0) == 1, is_active=d.get('isActive', 1) == 1, created_time=created_str, last_online=last_online_str, avatar=high_res if high_res else raw_avatar if raw_avatar else None, account_type=acct_label, raw_data=d)
