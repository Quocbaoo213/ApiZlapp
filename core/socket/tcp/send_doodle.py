...
def build_doodle_attach(
    ...
    """Tạo D3 Attachment JSON Dictionary cho Hình vẽ / Doodle (SUB 37)."""
    return build_photo_attach(
        url=url,
        width=width,
        height=height,
...
def build_doodle_message_packet(
    ...
    """Tạo Inner Packet gửi Doodle / Hình vẽ (CMD 207/113 SUB 37)."""
    return build_photo_message_packet(
        uid=uid,
        target_id=target_id,
        photo_url=doodle_url,
        seq=seq,
        ck_val=ck_val,
        cmsg_id=cmsg_id,
        is_group=is_group,
