...
def build_p110(sk_str: str, uid: int, vercode: int = 260801901) -> bytes:
    """Tạo payload 110 bytes chứa thông tin thiết bị và session key."""
    s = sk_str.encode('utf-8')
    inner = (b'\x01' + b'\xff' * 4 + b'\x00\x00' + struct.pack('<I', vercode)
             + b'\x02' + struct.pack('<H', 19) + b'samsung_SM-J610F_11'
             + struct.pack('<H', 6) + b'000000' + b'\x00' * 6
             + struct.pack('<H', len(s)) + s)
    le_t = struct.pack('<I', 133)
    le_m = struct.pack('<I', uid & 0xFFFFFFFF)
    key = bytes(XK[i % 32] ^ le_t[i % 4] ^ le_m[i % 4] for i in range(32))
    return bytes(inner[i] ^ key[i % 32] for i in range(110))


def build_authen_body(sk_str: str, uid: int, ctr: int = 0xFFFFFFFF) -> bytes:
    """Tạo authen body cho Frame 0 Handshake."""
    s = (uid + ctr + 1 + 3 + 5 + 1 + 1) & 0xFFFFFFFF
    cs = s ^ CKSUM_MAGIC
    return (struct.pack('<I', cs) + b'\x01\x01'
            + struct.pack('<I', ctr) + struct.pack('<I', uid)[::-1][:0] + b'' +
            struct.pack('<I', uid) + b'\x03' + b'\x01\x00\x05'
            + build_p110(sk_str, uid))


def build_prov_frame(
    session_key: str,
    dk: bytes,
    ksid: str,
    server_pubkey_b64: Any,
    uid: int
) -> bytes:
    """
    Sinh PROV Frame 0 (Type 8) - Trao đổi khóa bảo mật X25519 ECDH + AES-256-GCM 2 lớp.
    """
...
def build_init_frames(uid: int, dest: int = 335694418, vercode: int = 260801901) -> List[Tuple[int, int, int, int, bytes]]:
    """
    Sinh danh sách init frames gửi ngay sau Handshake để kích hoạt phiên Active.
    """
    VC = struct.pack('<I', vercode)
    dest_le = struct.pack('<I', dest & 0xFFFFFFFF)
    uid_le = struct.pack('<I', uid & 0xFFFFFFFF)
    ...
    # ── 10100 submit bundle (Signal identity) ──
    bundle_path = None
    ...
    add(271, 0, 0, struct.pack('<I', 0x0B))
    add(1715, 0, 0, b'')
    ...
    crc = _z.crc32(csv) & 0xFFFFFFFF
    gz = (b"\x1f\x8b\x08\x00\x00\x00\x00\x00\x00\x03" + body
          + struct.pack('<II', crc, len(csv) & 0xFFFFFFFF))
    add(1530, 0, 0, b'\x04' + struct.pack('<I', (now_ms >> 8) & 0xFFFFFFFF)
        + struct.pack('<I', 1) + struct.pack('<I', len(gz)) + gz)
    add(1915, 0, 0, b'\x01' + VC + b'\x00\x00')
    add(396, 4, 0, b'\x01' + VC + struct.pack('<I', 1) + struct.pack('<I', 100)
        + struct.pack('<I', 160) + b'\x00' * 4 + b'\x01')
    ...
    # ── 3400 E2EE devices JSON ──
    e2ee_json = (b'{"devices":{"masterId":"aade588b835d3813157f91c24d3b5394",'
                 b'"encIdentity":"BSZhp3pIB9XiGN6DxHr\\/Cf\\/egcZwhiXk67b\\/tqBgSFB",'
                 b'"lastUpdateTs":%d,'
                 b'"encSignature":"0idIG9lZ8c444kPv\\/axpIi5KiaMQ2Lcb\\/54Zs+kwphSHTr9hV3H\\/StaVL4klnyNtsEwtDiyDVlqmPBVWICBg==",'
                 b'"companions":[]}}' % now_ms)
    ...
    add(1251, 4, 0, b'\x00\x00\x01' + VC + b'\x00\x00' + dest_le
        + struct.pack('<I', 6) + struct.pack('<I', 0xF0) + b'\x00' * 3)
...
def send_init_frames(sock, dk: bytes, uid: int, dest: int = 335694418, vercode: int = 260801901, send_lock=None):
    """Gửi tuần tự chuỗi khung khởi tạo để Active phiên trên Gateway server."""
