...
def build_photo_attach(
    ...
    """Tạo Photo Attachment payload theo chuẩn Zalo Native D3 Attachment."""
...
def build_video_attach(
    ...
    """Tạo Video Attachment payload theo chuẩn Zalo D3 Attachment."""
    thumb = str(thumb_url or url)
    u = str(url)
    return {
        "title": str(title or ""),
...
def build_d3_payload_group(
    ...
    """
    Xây dựng cấu trúc nhị phân D3 cho tin nhắn nhóm (hỗ trợ Text thường, Quote trích dẫn, Mention, Style, Ảnh/Attach và TTL tự xóa).
    """
...
def build_group_message_packet(
    ...
    """Tạo Inner Packet gửi tin nhắn nhóm (CMD 207 SUB 1) hỗ trợ Plaintext, Style, Attach/Ảnh và TTL."""
    ...
    d3_plain = build_d3_payload_group(
        ...
        rtf_mode=rtf_mode, attach=attach
    )
...
def build_1to1_message_packet(
    ...
    """Tạo Inner Packet gửi tin nhắn 1-1 (CMD 113 SUB 41) hỗ trợ Plaintext, Style, RTF, Attach/Ảnh và TTL."""
...
def build_photo_binary_payload(
    url: str,
    title: str = "",
    description: str = "",
    thumb_url: Optional[str] = None,
    ...
    """
    Tạo Payload Nhị Phân Native Photo Message theo chuẩn C3 trong pn/g0.smali.
    ...
    body = (
        struct.pack('<H', len(a_bytes)) + a_bytes +
        struct.pack('<H', len(e_bytes)) + e_bytes +
        struct.pack('<H', len(d_bytes)) + d_bytes +
        struct.pack('<H', len(c_bytes)) + c_bytes +
        struct.pack('<H', 0) +
        struct.pack('<H', len(f_bytes)) + f_bytes +
...
def build_photo_message_packet(
    ...
    """
    Tạo Inner Packet gửi tin nhắn hình ảnh Native (CMD 207/113 SUB 32/37) hoặc D3 attachment fallback (SUB 1/41).
    """
    t_val = str(title or caption or "")
    d_val = str(description or photo_url)

    if not native:
        attach_obj = build_photo_attach(
            url=photo_url,
            width=width,
            height=height,
        ...
        if is_group:
            return build_group_message_packet(
                uid=uid,
                group_id=target_id,
                text=msg_text,
                seq=seq,
                ck_val=ck_val if ck_val is not None else 0,
                cmsg_id=cmsg_id,
                ttl_ms=ttl_ms,
            ...
            return build_1to1_message_packet(
                uid=uid,
    ...
    if ck_val is None or ck_val == 0:
        ck_val = compute_checksum(
            cmd=cmd, sub=sub, seq=seq, uid=uid,
            target=target_id, target_type=target_type, cmsg=cmsg_id,
            bb=1, ty=2, ver=3
        )
    hdr = InnerPacketHeader(ck=ck_val, bb=1, ty=2, seq=seq, uid=uid, ver=3, cmd=cmd, sub=sub)
    plain_payload = build_photo_binary_payload(
        url=photo_url,
        title=t_val,
        description=d_val,
        thumb_url=thumb_url,
    ...
    params_prefix = struct.pack('<IBII', target_id & 0xFFFFFFFF, target_type, cmsg_id & 0xFFFFFFFF, 416)
    return hdr.pack() + params_prefix + xor_payload


def build_video_binary_payload(
    url: str,
    title: str = "",
    description: str = "",
    thumb_url: Optional[str] = None,
    ...
    """
    Tạo Payload Nhị Phân Native Video Message theo chuẩn C3 trong pn/g0.smali và Lp00/x0.smali.
    """
    thumb = str(thumb_url or url)
    u = str(url)
    t = str(title or "")
    ...
    meta_dict = {
        ...
        "video_original_width": int(width or 1280),
        "video_original_height": int(height or 720),
        "video_width": int(width or 1280),
        "video_height": int(height or 720),
        "video_rotation": 0,
        "isHD": 1
    }
    if total_size > 0:
...
def build_video_message_packet(
    ...
    """
    Tạo Inner Packet gửi tin nhắn Video Native (CMD 207/113 SUB 32/37) hoặc D3 attachment fallback (SUB 1/41).
    """
    t_val = str(title or caption or "")
    ...
    hdr = InnerPacketHeader(ck=ck_val, bb=1, ty=2, seq=seq, uid=uid, ver=3, cmd=cmd, sub=sub)
    plain_payload = build_video_binary_payload(
        url=video_url,
        title=t_val,
        description=d_val,
        thumb_url=thumb_url,
...
def build_join_group_by_link_901_packet(
    uid: int,
    link_url: str,
    seq: int = -100,
    ck_val: int = 0,
    ...
    """
    Tạo Inner Packet tham gia nhóm qua link (CMD 901 SUB 3).
    ...
    # Apply 4th-byte XOR 0x33 encoding to URL bytes (Zalo-specific obfuscation)
...
def build_preview_link_901_packet(
    ...
    """Alias → build_join_group_by_link_901_packet (chuẩn PCAP 16/09/2026)."""
...
def parse_d3_compressed_json(body: bytes, uid: int) -> Optional[Dict[str, Any]]:
    """
    Giải mã payload JSON nén Deflate/Gzip D3 từ phản hồi Socket (CMD 244, 901, 902, etc.).
    Hỗ trợ cả trường hợp body có tiền tố 4-byte status code lẫn không có.
    """
    if not body or len(body) < 10:
        return None
    import zlib, re, json
    ule = (uid & 0xFFFFFFFF).to_bytes(4, 'little')
