...
class GroupActionsMixin:
    """Mixin xử lý các thao tác quản trị và tương tác nhóm qua Socket."""
    ...
    def leave_group(
        ...
        try:
            ...
            # 1. Gói tin rời nhóm trực tiếp CMD 225 SUB 3
            ...
            # 2. Gói tin rời nhóm phụ trợ CMD 239 SUB 1
    ...
    def add_member(
        ...
        """Thêm / mời thành viên vào nhóm chat qua Socket (CMD 235 SUB 0)."""
        if not self.is_connected or not self.sock:
            return False
        try:
            if isinstance(member_uids, (int, str)):
    ...
    def preview_link_901(
        ...
        """Gửi yêu cầu phân giải / Preview link nhóm (CMD 901 SUB 3)."""
        if not self.is_connected or not self.sock:
            return {'sent': False, 'error': 'Not connected'} if wait_response else False
        try:
    ...
    def query_group_906(
        ...
        """Gửi yêu cầu truy vấn cấu hình nhóm theo GID (CMD 906 SUB 0)."""
        if not self.is_connected or not self.sock:
            return {'sent': False, 'error': 'Not connected'} if wait_response else False
        try:
            seq, cmsg, _ = self._get_next_counters()
    ...
    def join_group_by_link(
        ...
        """Tham gia nhóm qua link (CMD 901 SUB 3)."""
        if not self.is_connected or not self.sock:
            return {'sent': False, 'error': 'Not connected'} if wait_response else False
