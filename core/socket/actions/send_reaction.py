...
from core.socket.protocol import compute_checksum, build_reaction_packet, build_outer_frame
logger = logging.getLogger('core.socket.actions.send_reaction')

class SendReactionActionMixin:

    def send_reaction(self, target_id: int, cli_msg_id: int, global_msg_id: int=0, icon: str='❤️', is_group: bool=True) -> bool:
        ...
        try:
            ...
            inner = build_reaction_packet(uid=self.uid, target_id=int(target_id), cli_msg_id=int(cli_msg_id), global_msg_id=int(global_msg_id or 0), icon=icon, is_group=is_group, seq=seq, ck_val=ck, cmsg_id=cmsg)
