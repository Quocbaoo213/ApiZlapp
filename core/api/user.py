...
class UserAPI:
    """Quản lý tra cứu hồ sơ người dùng Zalo (Bạn bè, Người lạ, Thành viên nhóm)."""
    ...
    def register_seen_user(self, uid: int | str, display_name: Optional[str] = None, avatar: Optional[str] = None):
        """Học thông tin người dùng được phát hiện từ sự kiện realtime."""
        try:
            u_int = int(uid)
        ...
        if u_int not in self._seen_users:
            self._seen_users[u_int] = {
                "userId": u_int,
                "displayName": display_name or f"Thành viên {u_int}",
                "avatar": avatar or "",
    ...
    def get_user_profile(
        ...
        """Truy xuất UserProfile cho bất kỳ UID nào (Bạn bè hoặc Người lạ)."""
        try:
            uid = int(target_uid)
        except (ValueError, TypeError):
            return None
    ...
    def get_all_friends(self, force_refresh: bool = False) -> List[UserProfile]:
        """Lấy toàn bộ danh sách bạn bè đã cache."""
    ...
    def get_aliases(self) -> Dict[int, str]:
        """Lấy bảng tra cứu biệt danh (Alias map)."""
        return dict(self._aliases)

    def discover_contacts(self, phones: List[str]) -> Optional[Dict[str, Any]]:
        """Khám phá liên hệ Zalo theo danh sách số điện thoại."""
        sess = self._load_session()
        if not sess:
            return None
        ...
        req = urllib.request.Request(
            "https://tapi.api.zaloapp.com/api/contact/discoverContact",
