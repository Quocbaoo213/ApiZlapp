...
class BaseAPI:
    """Base class cho tất cả các sub-service API trong hệ thống."""
    ...
    def set_socket_client(self, socket_client):
        """Gắn instance SocketClient đang hoạt động."""
        self._socket = socket_client
    ...
    def _extract_session_fields(self, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Trích xuất các trường xác thực từ dictionary dữ liệu session."""
    ...
    def _load_session(self) -> Optional[Dict[str, Any]]:
        """Nạp thông tin phiên đăng nhập (Hỗ trợ dict in-memory hoặc tìm kiếm đa nền tảng)."""
