...
class GroupInfoAPI(BaseAPI):
    """Quản lý tra cứu thông tin nhóm và phân giải link tham gia nhóm."""

    @staticmethod
    def parse_group_target(target: Union[str, int]) -> Tuple[str, Optional[int], Optional[str]]:
        """Phân tích chuỗi target thành ('link', gid, link_code) hoặc ('id', group_id, None)."""
        if isinstance(target, int):
            return "id", target, None

        target_str = str(target).strip()
        m_link = re.search(r'(?:https?://)?(?:(?:www\.|chat\.)?zalo\.me)/g/([a-zA-Z0-9_\-]+)', target_str)
        link_code = m_link.group(1) if m_link else None

        # Check for explicit numeric GID in target string (6-15 digits)
        found_gid = None
        digits_matches = re.findall(r'\b(\d{6,15})\b', target_str)
        if digits_matches:
    ...
    def resolve_group_id_from_link(self, link_or_code: str) -> Optional[int]:
        """Phân giải liên kết nhóm (https://zalo.me/g/... hoặc link code) để lấy Group ID."""
        ...
        # 1. Kiểm tra cache trong socket client nếu có
        ...
        # 2. Gửi yêu cầu phân giải qua Socket CMD 244 SUB 4
    ...
    def format_relative_created_time(created_time_ms: Union[int, float, str]) -> str:
        """
        Định dạng chuỗi thời gian tạo tương đối theo thuật toán của App Zalo
        ...
        try:
            ...
            if ts < 1e11:  # nếu là seconds thì đổi sang milliseconds
                ts = ts * 1000.0
            import time
            now_ms = time.time() * 1000.0
            diff = max(0.0, now_ms - ts)
            ONE_DAY_MS = 86400000.0  # 0x5265c00
            ONE_MONTH_MS = 2635200000.0  # 0x9d11f600 (~30.5 ngày)
            ONE_YEAR_MS = 31536000000.0  # 365 ngày
    ...
    def preview_group_link(self, link_or_code: str) -> Dict[str, Any]:
        """
        Xem trước thông tin nhóm / cộng đồng từ Link (tương ứng GroupLobbyView trong app Zalo).
        ...
        # 1. Phân giải qua Socket CMD 901 SUB 3 (Preview Link)
        sock_res = None
        if self._socket and getattr(self._socket, 'is_connected', False):
            try:
        ...
        res_gid = (
            gid or
            (sock_res.get('group_id') if isinstance(sock_res, dict) else None) or
            http_data.get('groupId') or
            http_data.get('grid') or
            http_data.get('id')
        )

        name = (
            http_data.get('name') or
            http_data.get('groupName') or
            http_data.get('gname') or
            f"Nhóm {res_gid or link_code}"
        )
        ...
        requires_approval = bool(
            (isinstance(http_data.get('setting'), dict) and http_data['setting'].get('joinAppr')) or
            http_data.get('requireApproval') or
            http_data.get('needApprove')
        )
        ...
        # Định dạng text hiển thị chuẩn UI GroupLobbyView của App Zalo
        type_str = "Cộng đồng" if is_community else "Nhóm"
        lines = [
            f"🏷️ [{type_str.upper()} ZALO] {name}",
            f"🔗 Liên kết: {link_full}",
        ]
        if res_gid:
        ...
        return {
            "success": bool(res_gid or http_data or (sock_res and sock_res.get('sent'))),
            "group_id": int(res_gid) if res_gid else None,
            "name": name,
            "desc": desc,
            "avatar": avatar,
            ...
            "raw_data": http_data or sock_res
        }
