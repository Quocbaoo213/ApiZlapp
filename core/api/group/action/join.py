...
class JoinAPI(BaseAPI):
    """Quản lý tham gia nhóm qua Socket (CMD 901 SUB 3, CMD 2000 SUB 3, CMD 246 SUB 3, CMD 244 SUB 4, CMD 250 SUB 1)."""
    ...
    def join_group_by_id(self, group_id: Union[int, str], msg: str = "", source: int = 1) -> Dict[str, Any]:
        """Tham gia nhóm Zalo qua Group ID / UID (CMD 2000 SUB 3, CMD 246 SUB 3, CMD 244 SUB 4)."""
        gid = int(group_id)
        socket_sent = False
        ...
        return {
            "success": socket_sent or bool(res_http and res_http.get('error_code') == 0),
            "type": "id",
            "group_id": gid,
            "status": "Đã gửi yêu cầu tham gia nhóm qua ID thành công" if (socket_sent or res_http) else "Gửi yêu cầu thất bại",
    ...
    def join_group_by_link(self, link_or_code: str, msg: str = "", source: int = 1, group_id: Union[int, str] = 0) -> Dict[str, Any]:
        """Tham gia qua link. Giữ FULL URL https://zalo.me/g/... + gid đã biết (nếu có)."""
        ...
        if self._socket and getattr(self._socket, 'is_connected', False):
            try:
                # 1. Gửi CMD 901 SUB 3 (Link Preview & Interaction)
                res_901 = self._socket.join_group_by_link(
                    link_url=link_full,
                    group_id=gid,
                    msg=msg,
                    source=source,
                ...
                # 2. Gửi CMD 244 SUB 4 (Request Join Group qua Link/Code)
                res_244 = self._socket.request_join_group(
                    group_id=gid or 0,
                    link_url=link_full,
                    msg=msg,
                    source=source,
                ...
                # 3. Nếu đã xác định được GID, gửi thêm CMD 2000 SUB 3
                if gid:
    ...
    def join_group(self, target: Union[str, int], msg: str = "", source: int = 1) -> Dict[str, Any]:
        """Phương thức vạn năng: tự nhận diện Link URL hoặc Group ID."""
