...
def main():
    parser = argparse.ArgumentParser(
        prog="python3 -m core.login",
        description="Zalo Mobile Login CLI v10 (Auto-Captcha + 2FA / Friend Verify + PROV Authen Probe)"
    )
    ...
    auth = PasswordAuth(
        zcid=args.zcid,
        zcid_path=args.zcid_path,
        renew_zcid=args.renew_zcid
    )
    ...
    try:
        session = auth.authenticate(
            ...
            out_session_path=args.out,
            zaloprefs_path=args.zaloprefs,
            target_uid=args.uid,
            probe_socket=(not args.no_probe)
        )
        print("\n" + "=" * 50)
