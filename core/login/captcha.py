...
def solve_captcha_img(bg_png: bytes, slice_png: bytes, top: int) -> Tuple[int, float]:
    """
    Giải slide captcha: correlation matching giữa slice và bg tại y=top.
    ...
    # Lấy các điểm pixel có độ mờ > 200 trong slice để làm mẫu so khớp
    samples = [
        (i, j)
        for i in range(0, sl.size[0], 3)
        for j in range(0, sl.size[1], 3)
        if sp[i, j][3] > 200
    ]
...
def captcha_flow(captcha_session: str, zcid: str = "") -> Optional[str]:
    """
    Tự động thực hiện chu trình: gen challenge -> solve -> verify -> captchaToken.
    ...
    # 1. Tải Captcha Challenge (Background PNG + Slice PNG)
    ...
    # 3. Gửi tọa độ xác thực
    start = int(time.time() * 1000)
    body = urllib.parse.urlencode({
        'left': left,
        'startTime': start,
        'stopTime': start + 2200 + (left % 700)
    })
