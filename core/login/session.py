...
def extract_all(response: dict) -> dict:
    ...
    if val_b64:
        try:
            ...
            dk_hex = str(val_b64)
    return {'error_code': response.get('error_code'), 'dk_hex': dk_hex, 'ksid': ks.get('keySetId', ''), 'session_key': d.get('session_key', ''), 'uid': str(d.get('user_id', '')), 'cryptkey': d.get('CrypKey', ''), 'sign': d.get('sign', ''), 'token': d.get('token', ''), 'servers': [s for s in d.get('uploadServers') or d.get('socketServers') or [] if ':' not in s.get('host', '')], 'raw_session_key_len': len(d.get('session_key', ''))}
...
def read_from_zaloprefs(prefs_path: str, uid: str | int) -> Tuple[bytes, str]:
    ...
    try:
        kv = dbp.execute('SELECT value FROM prefs_v2 WHERE key=?', (f'KEY_STRING_SOCKET_PRO_V1_KEY_SET_VALUE_{uid}',)).fetchone()
        kid = dbp.execute('SELECT value FROM prefs_v2 WHERE key=?', (f'KEY_STRING_SOCKET_PRO_V1_KEY_SET_ID_{uid}',)).fetchone()
